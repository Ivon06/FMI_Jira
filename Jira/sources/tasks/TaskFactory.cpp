...
Task TaskFactory::createTask(const std::string& title, const std::string& description, TaskType type, TaskPriority priority, unsigned int creatorId) {
    TaskValidator::validateTitle(title);
    
    return Task(title, description, type, priority, creatorId);
}
